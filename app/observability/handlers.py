...
def record_metric(event: GatewayEvent) -> None:
    if isinstance(event, RequestStartedEvent):
        ...
        prometheus_metrics.requests_in_flight.labels(team_id=event.team_id).inc()
        ...
        prometheus_metrics.requests_total.labels(status="success", team_id=event.team_id).inc()
        prometheus_metrics.requests_in_flight.labels(team_id=event.team_id).dec()
        ...
        prometheus_metrics.requests_total.labels(status="failed", team_id=event.team_id).inc()
        prometheus_metrics.requests_in_flight.labels(team_id=event.team_id).dec()
